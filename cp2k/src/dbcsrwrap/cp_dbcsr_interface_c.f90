  ...
  FUNCTION make_conformant_scalar_c (scalar, matrix) RESULT (encapsulated)
    ...
    IF (scalar_data_type .EQ. dbcsr_type_complex_4 .OR.&
       ...
       IF(.NOT.(data_type .EQ. dbcsr_type_complex_4 .OR.&
            ...
            STOP "make_conformant_scalar_c: Can not conform a complex to a real number"
    END IF
...
! *****************************************************************************
  SUBROUTINE cp_iterator_next_2d_block_c (iterator, row, column,&
    ...
    CALL dbcsr_iterator_next_block (iterator, row, column,&
       block, transposed,&
       block_number, row_size, col_size, row_offset, col_offset)
    IF(transposed) STOP "cp_iterator_next_2d_block_c: CP2K does not handle transposed blocks."
...
! *****************************************************************************
  SUBROUTINE cp_iterator_next_1d_block_c (iterator, row, column, block,&
    ...
    IF(transposed) STOP "cp_iterator_next_1d_block_c: CP2K does not handle transposed blocks."
...
! *****************************************************************************
  SUBROUTINE cp_dbcsr_get_2d_block_p_c (matrix,row,col,block,found,&
    ...
    CALL dbcsr_get_block_p(matrix%matrix,row,col,block,tr,found,&
         row_size, col_size)
    IF(tr) STOP "cp_dbcsr_get_2d_block_p_c: CP2K does not handle transposed blocks."
...
! *****************************************************************************
  SUBROUTINE cp_dbcsr_get_block_p_c (matrix,row,col,block,found,&
    ...
    CALL dbcsr_get_block_p(matrix%matrix,row,col,block,tr,found,&
       row_size, col_size)
    IF(tr) STOP "cp_dbcsr_get_block_p_c: CP2K does not handle transposed blocks."
